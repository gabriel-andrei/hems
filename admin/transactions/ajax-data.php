    ...
    if ($type == 'service'){
        ...
        $result = $conn->query("SELECT id, name, price
            ...
            , coalesce((SELECT SUM(tp.qty) FROM `transaction_products` tp 
                            ...
                            where tp.product_id = product_list.id and tl.status != 3 AND tp.`transaction_id` <> '{$id}'),0) used
        ...
        if ($result && $result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                ...
                echo '<option value="'.$row['id'].'" data-price="'.$row['price'].'"  data-available="'.$product_stocks.'" >'.$row['name'].' @ ₱'.number_format($row['price'], 2).' ('.$product_stocks.' items left)</option>';
            }
        }else
        echo '<option value="" ></option>';
    }
